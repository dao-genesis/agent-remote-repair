...
// ═══════════════════════════════════════════════════════════════
// 演化·三明治核心 — 本源 dao-bridge “穿透它自己这一台” → 升级为
// “公网任意设备汇入单一中枢”：命令队列 + 长轮询 + agent_id 路由 + 一行 bootstrap。
// 被控端一行接入： irm <hub>/api/bootstrap.ps1 | iex
// ═══════════════════════════════════════════════════════════════
...
function runSelf(cmd, cwd, timeoutMs) {
  return new Promise((resolve) => {
...
// 被控端一行接入脚本（动态注入当前隔离 URL）— 长轮询命令队列，回传结果。
...
class WorkspaceServer {
  ...
  // ── 演化·中枢路由：被控端登记 + 每 agent 命令队列/结果表/唤醒器 ──
  ...
  deviceListMd(wsInfo) {
    const rows = [["（空）/ " + wsInfo.host, "中枢本机", wsInfo.platform, "★ 中枢"]];
    for (const a of this.agentListFull()) {
      if (a.status !== "online") continue;
      rows.push([a.id, a.hostname, a.os, a.user]);
    }
    return "| agent_id | 主机名 | 系统 | 备注/用户 |\n|---|---|---|---|\n" +
...
class Bridge {
  ...
  startWatchdog() {
    ...
    let iv = 20000, th = 2;
    try { const c = daoCfg(); iv = parseInt(c.get("watchdogIntervalMs"), 10) || 20000; th = parseInt(c.get("watchdogFailThreshold"), 10) || 2; } catch (e) {}
  ...
  // 演化：被控端接入/掉线 → 重写云端 MD + 推送前端实时刷新
  onAgentsChanged() { try { this.writeArtifacts(); } catch (e) {} this.notify(); }
  bootstrapCmd() {
    const u = this.url || ("http://127.0.0.1:" + this.srv.port);
    return "irm " + u + "/api/bootstrap.ps1 | iex";
  }
  state() {
    return {
  ...
  generateCloudAgentMd() {
    ...
    return [
      ...
      "| 在线被控端数 | " + this.srv.agentListFull().filter((a) => a.status === "online").length + " |",
      "| 更新于 | " + ts + " |",
      "",
      "## 当前在线设备（中枢 + 被控端·随接入实时刷新）",
      "",
      this.srv.deviceListMd(wsInfo),
      "",
      "> `agent_id` 留空/`self`/`local`/中枢主机名 = 操控**中枢本机**；填某台**主机名** = 操控对应被控端。",
      "> 被控端一行接入（任意 Windows）：`" + this.bootstrapCmd() + "`",
      "",
      "## API 参考",
      "",
...
class BridgeViewProvider {
  ...
  resolveWebviewView(view) {
    ...
    const iv = setInterval(() => { try { this.post({ type: "state", state: this.bridge.state() }); } catch (e) {} }, 3000);
...
<div class="card">
  ...
  <div class="row" style="margin-top:6px">
    <button data-op="copyAll" title="一键复制公网 URL 与 Token（含 Authorization 头），直接粘贴给云端 Agent">📋 复制</button>
    <button data-op="restart" title="重启隧道（URL 会变，Token 不变）">♻️ 重启隧道</button>
...
window.addEventListener('message',(e)=>{const m=e.data;
  if(m.type==='state'){const s=m.state||{};const on=!!s.url;
    ...
    var ags=s.agents||[];var onAgs=ags.filter(function(a){return a.status==='online';});
    var rows='<tr><td><code>（空）</code></td><td>'+esc(s.host||(s.ws&&s.ws.host)||'')+' ★中枢</td><td>本机</td></tr>';
    onAgs.forEach(function(a){rows+='<tr><td><code>'+esc(a.id)+'</code></td><td>'+esc(a.hostname)+'</td><td>'+esc(a.os)+'</td></tr>';});
    document.getElementById('devs').innerHTML='<div class="muted">在线设备（'+onAgs.length+' 被控端 + 1 中枢）</div><table style="width:100%;border-collapse:collapse;font-size:11px;margin-top:2px"><tr style="color:var(--vscode-descriptionForeground)"><td>agent_id</td><td>主机名</td><td>系统</td></tr>'+rows+'</table>';
